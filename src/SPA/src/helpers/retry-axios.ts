...
function onError(err: AxiosError) {
	...
	config.retry = typeof config.retry === 'number' ? config.retry : 2;
	...
	config.maxRetryAfter = typeof config.maxRetryAfter === 'number' ? config.maxRetryAfter : 60000 * 5;
	...
	const onBackoffPromise = new Promise((resolve, reject) => {
		...
		if (config.checkRetryAfter && err.response && err.response.headers && err.response.headers['retry-after']) {
		...
		const retrycount = (err.config as RaxConfig).raxConfig!.currentRetryAttempt!;
	...
	return Promise.resolve()
		...
		.then(() => {
			return config.instance!.request(err.config).catch(err => {
				if (!config.disableToaster && config.currentRetryAttempt == (config.retry || 0) - 1) {
					toast('Operation failed unexpectedly', { type: 'error' });
					console.error(err);
				}
				throw err;
			});
